# Final Premium Bulk View
# Export to CSV, Settings near results, Sort toggle
...
print(f"Final enhancement at {start_idx}-{func_end}")
...
new_component = '''        function BulkQuickView({ priceRules, courierRules, active }) {
            ...
            const sortedRows = useMemo(() => {
                if (sortBy === 'margin-asc') return [...rows].sort((a, b) => a.marginPct - b.marginPct);
                if (sortBy === 'margin-desc') return [...rows].sort((a, b) => b.marginPct - a.marginPct);
                if (sortBy === 'price') return [...rows].sort((a, b) => b.sell - a.sell);
                return rows;
            }, [rows, sortBy]);
            ...
            const exportToCSV = () => {
                const headers = ['#', 'Buy (USD)', 'Dims (cm)', 'Pack', 'Landed (GBP)', 'Target Sell', 'Margin %', 'Profit', 'Units/40HQ', 'Courier', 'Courier Cost'];
                const csvRows = rows.map(r => [
                    r.i, r.unitUSD.toFixed(2), r.dims, r.pack, r.landedUnit.toFixed(2), r.sell.toFixed(2), r.marginPct.toFixed(1), r.profit.toFixed(2), r.units, r.courier?.carrier || '', r.courier?.cost?.toFixed(2) || ''
                ]);
                ...
                <div className="space-y-6">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <div className="lg:col-span-7 space-y-4">
                            {/* Settings + Actions Bar (TOP of results) */}
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-4">
                                    ...
                                    {rows.length > 0 && (
                                        <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="glass-input rounded-lg px-3 py-1.5 text-xs font-bold">
                                            ...
                                            <option value="margin-asc">Margin \\u2191 (Low first)</option>
                                            <option value="margin-desc">Margin \\u2193 (High first)</option>
                            ...
                            {rows.length > 0 ? (
                                <>
                                    ...
                                    <div className="space-y-3 max-h-[55vh] overflow-y-auto pr-1">
                                        {sortedRows.map(r => (
                                            <div key={r.i} onClick={() => setExpandedRow(expandedRow === r.i ? null : r.i)} className={`glass-card rounded-xl overflow-hidden cursor-pointer transition-all duration-200 hover:shadow-lg border-l-4 ${statusColors[r.status].border} ${statusColors[r.status].bg}`}>
                                                <div className="p-4 flex items-center gap-4">
                                                    ...
                                                    <div className={`transition-transform duration-200 ${expandedRow === r.i ? 'rotate-180' : ''}`}><svg className="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div>
                                                </div>
...
print(f"Final Premium Bulk View!")
print(f"Script tags: {check.count('<script')} open, {check.count('</script>')} close")
print(f"Added: Export to CSV, Settings at top of results, Sort dropdown")
