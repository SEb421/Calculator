# Enhanced Premium Bulk View
# Card-based results, mobile sticky, animations, copy-to-clipboard
...
print(f"Enhancing BulkQuickView at {start_idx}-{func_end}")
...
new_component = '''        function BulkQuickView({ priceRules, courierRules, active }) {
            ...
            const rows = useMemo(() => {
                ...
                return prices.map((priceLine, i) => {
                    ...
                    return { i: i + 1, unitUSD, pack, L, W, H, weight, vol, units, cartons, landedUnit, landedPack, sell, profit, marginPct, courier, status };
                }).filter(Boolean);
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <div className="lg:col-span-5 space-y-5">
                            {/* Primary: Unit Prices with better CTA */}
                            <div className="relative">
                                ...
                                <div className="relative glass-card rounded-2xl p-5 ring-2 ring-blue-400/60 dark:ring-indigo-400/60 shadow-xl shadow-blue-500/10">
                                    ...
                                    <textarea 
                                        className="w-full h-40 glass-input rounded-xl p-4 text-base font-bold !bg-blue-50/70 dark:!bg-indigo-900/40 font-mono leading-relaxed resize-none placeholder:text-blue-300 dark:placeholder:text-indigo-400 focus:ring-2 focus:ring-blue-400 transition-all" 
                                        placeholder="2.35&#10;3.50&#10;4.20, 2  \\u2190 price, pack"
                                        value={pricesText}
                            ...
                            <div className="glass-card rounded-2xl p-5 hover:shadow-lg transition-shadow">
                                ...
                                <textarea 
                                    className="w-full h-32 glass-input rounded-xl p-4 text-sm font-mono leading-relaxed resize-none focus:ring-2 focus:ring-gray-300 transition-all" 
                            ...
                            <div className="glass-card rounded-2xl overflow-hidden hover:shadow-md transition-shadow">
                                ...
                                <div className={`overflow-hidden transition-all duration-300 ${showAdvanced ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <div className="px-4 pb-4 space-y-3 border-t border-gray-100 dark:border-gray-800 pt-3">
                                        <div>
                                            <label className="text-[10px] font-medium text-gray-400 mb-1 block uppercase">Pack Size</label>
                                            <textarea className="w-full h-16 glass-input rounded-lg p-2 text-sm font-mono resize-none" placeholder="1&#10;2" value={packText} onChange={e => setPackText(e.target.value)} />
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-medium text-gray-400 mb-1 block uppercase">Weight (kg)</label>
                                            <textarea className="w-full h-16 glass-input rounded-lg p-2 text-sm font-mono resize-none" placeholder="2.5&#10;3.0" value={weightText} onChange={e => setWeightText(e.target.value)} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            {rows.length > 0 ? (
                                <>
                                    ...
                                    <div className="flex items-center justify-between gap-4">
                                        <div className="flex items-center gap-3">
                                            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-green-100 dark:bg-green-900/30">
                                                ...
                                                <span className="text-sm font-bold text-green-700 dark:text-green-300">{stats.good}</span>
                                            </div>
                                            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-100 dark:bg-amber-900/30">
                                                <span className="w-2 h-2 rounded-full bg-amber-500"></span>
                                                <span className="text-sm font-bold text-amber-700 dark:text-amber-300">{stats.review}</span>
                                            </div>
                                            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-100 dark:bg-red-900/30">
                                                <span className="w-2 h-2 rounded-full bg-red-500"></span>
                                                <span className="text-sm font-bold text-red-700 dark:text-red-300">{stats.low}</span>
                                            </div>
                                        </div>
                                        <button onClick={copyAllPrices} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${copied ? 'bg-green-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200'}`}>
                                    ...
                                    <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
                                        {rows.map(r => (
                                            <div 
                                                key={r.i} 
                                                onClick={() => setExpandedRow(expandedRow === r.i ? null : r.i)}
                                                className={`glass-card rounded-xl overflow-hidden cursor-pointer transition-all duration-200 hover:shadow-lg border-l-4 ${statusColors[r.status].border} ${statusColors[r.status].bg}`}
                                            >
                                                <div className="p-4 flex items-center gap-4">
                                                    ...
                                                    {/* Expand Icon */}
                                                    <div className={`transition-transform duration-200 ${expandedRow === r.i ? 'rotate-180' : ''}`}>
                                                ...
                                                <div className={`overflow-hidden transition-all duration-200 ${expandedRow === r.i ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                                                    <div className="px-4 pb-4 pt-2 border-t border-gray-200/50 dark:border-gray-700/50 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                                                        ...
                                                        <div><span className="text-gray-400 block">Dims</span><span className="font-bold">{r.L}x{r.W}x{r.H}cm</span></div>
                                ...
                                <div className="glass-card rounded-2xl p-12 text-center">
                                    <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 flex items-center justify-center">
                                        <svg className="w-8 h-8 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                                    </div>
                        ...
                        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-t border-gray-200/50 dark:border-gray-700/50 px-4 py-3 safe-area-bottom">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="text-right">
                                    ...
                                    <div className="text-lg font-bold text-gray-900 dark:text-white">{stats.avgMargin.toFixed(1)}%</div>
                                </div>
                            </div>
                        </div>
                    )}
...
print(f"Enhanced Premium Bulk View!")
print(f"Script tags: {check.count('<script')} open, {check.count('</script>')} close")
print(f"New: Card-based results, mobile sticky, copy prices, expand rows, better empty state")
